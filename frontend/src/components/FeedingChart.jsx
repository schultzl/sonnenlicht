...
export default function FeedingChart({ child }) {
  ...
  useEffect(() => {
    api.getFeedingGuide().then(setTable).catch((e) => setError(e.message))
  }, [])
  ...
  return (
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b border-gray-100">
              ...
              <th className="px-4 py-3 font-medium">Mahlzeiten pro Tag</th>
              <th className="px-4 py-3 font-medium">Menge pro Mahlzeit</th>
            </tr>
          </thead>
          <tbody>
            {table.map((row) => {
              ...
              return (
                ...
                >
                  <td className="px-4 py-3 font-medium text-gray-900">
                    Woche {row.week_from}
                    {row.week_to !== row.week_from && `–${row.week_to}`}
                    {isActive && (
                  ...
                  <td className="px-4 py-3 text-gray-700">
                    {row.meals_min === row.meals_max
                      ? row.meals_min
                      : `${row.meals_min}–${row.meals_max}`}
                  </td>
                  <td className="px-4 py-3 text-gray-700">
                    {row.volume_min_ml}–{row.volume_max_ml} ml
                  </td>
                </tr>
              )
            })}
      ...
      <p className="text-xs text-gray-400 px-1">
        Richtwerte gelten vor allem für Flaschennahrung (z. B. Pre-Milch) — gestillte Kinder
        trinken meist nach Bedarf statt zu festen Mengen.
      </p>
    </div>
  )
}
